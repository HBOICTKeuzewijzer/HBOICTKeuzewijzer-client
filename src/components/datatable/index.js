...
export class Datatable extends CustomElement {
    ...
    #constructTable() {
        ...
        if (this.#config.paging) {
            ...
            pagingContainer
                ...
                .forEach(button => this.trackListener(button, 'click', event => this.#onPaging(event)))
        } else {
    ...
    #loadQueryParams() {
        if (this.#config.paging) {
            ...
            this.#queryState.pageSize = this.#config.pageSize
        }

        if (this.#config.searching) {
            this.#queryState.filter = ''
        }

        if (this.#config.columns.some(column => column.sorting === true)) {
            this.#queryState.sortColumn = ''
    ...
    async #getData() {
        ...
        for (const state in this.#queryState) {
            params.append(state, this.#queryState[state])
        }
